	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp2.xyz = _SourcePoint - inp.texcoord2.xyz;
                tmp1.w = dot(tmp2.xyz, tmp2.xyz);
                tmp2.x = sqrt(tmp1.w);
                tmp1.w = tmp1.w * tmp2.x;
                tmp1.w = 1.0 / tmp1.w;
                tmp1.w = tmp1.w * _DistortStrength;
                tmp2 = tex2D(_NoiseTex, inp.texcoord.xy);
                tmp1.w = tmp1.w * _ShakeStrength;
                tmp2.zw = tmp1.ww * _Shake;
                tmp2.xy = -tmp2.zw * tmp2.xy + inp.texcoord.xy;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp3.xyz = _SourcePoint - inp.texcoord2.xyz;
                tmp1.w = dot(tmp3.xyz, tmp3.xyz);
                tmp2.w = sqrt(tmp1.w);
                ...
                tmp1.w = tmp1.w * _ShakeStrength;
                tmp3.zw = tmp1.ww * _Shake;
                tmp3.xy = -tmp3.zw * tmp3.xy + inp.texcoord.xy;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.x = 1.0 / tmp0.x;
                tmp0.x = tmp0.x * _DistortStrength;
                tmp0.x = tmp0.x * _ShakeStrength;
                tmp0.xy = tmp0.xx * _Shake;
                tmp1 = tex2D(_NoiseTex, inp.texcoord.xy);
                tmp0.xy = -tmp0.xy * tmp1.xy + inp.texcoord.xy;
