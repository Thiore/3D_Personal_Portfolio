	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                o.texcoord.xy = v.texcoord.xy * _DissolveTex_ST.xy + _DissolveTex_ST.zw;
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.x = -_DissolveAmount * 1.1 + tmp0.x;
                tmp0.y = tmp0.x <= -0.1;
                if (tmp0.y) {
                    discard;
                }
                tmp0.x = saturate(-0.1 / tmp0.x);
                tmp0.yzw = _CrackColor.xyz - _Color.xyz;
                ...
                tmp2.xyz = inp.texcoord1.xyz * -tmp1.www + -tmp1.xyz;
                ...
                tmp1.w = saturate(dot(tmp4.xyz, _WorldSpaceLightPos0.xyz));
                tmp0.w = saturate(tmp0.w);
                tmp4.x = dot(tmp1.xyz, _WorldSpaceLightPos0.xyz);
                tmp4.y = 1.0 - tmp0.w;
                tmp4.zw = tmp4.xy * tmp4.xy;
                tmp1.xy = tmp4.xy * tmp4.xw;
                tmp1.xy = tmp4.zy * tmp1.xy;
                tmp1.z = 1.0;
                tmp4 = tex2D(unity_NHxRoughness, tmp1.xz);
                tmp0.w = tmp4.x * 0.64;
                tmp0.xyz = tmp0.xyz * float3(0.96, 0.96, 0.96) + tmp0.www;
                tmp1.xzw = tmp1.www * tmp3.xyz;
                tmp0.w = tmp1.y * 0.0 + 0.04;
                tmp2.xyz = tmp0.www * tmp2.xyz;
                o.sv_target.xyz = tmp0.xyz * tmp1.xzw + tmp2.xyz;
                o.sv_target.w = 1.0;
                return o;
			}
			ENDCG
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.w = saturate(dot(tmp4.xyz, tmp1.xyz));
                ...
                tmp1.x = tmp1.x * 0.64;
                tmp0.xyz = tmp0.xyz * float3(0.96, 0.96, 0.96) + tmp1.xxx;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.xyz = _Color.xyz * _MainGlow.xxx;
                o.sv_target3.xyz = tmp0.xyz * _Glow.xxx + tmp1.xyz;
                o.sv_target.w = 1.0;
