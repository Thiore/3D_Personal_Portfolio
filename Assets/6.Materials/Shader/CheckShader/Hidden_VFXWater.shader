	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                fout o;
                float4 tmp0;
                tmp0.xy = inp.texcoord.xy * _ParticleTextureA_ST.xy + _ParticleTextureA_ST.zw;
                tmp0 = tex2D(_ParticleTextureA, tmp0.xy);
                tmp0.x = dot(float3(_Opacity.x, _R.x, _G.x), tmp0.xyz);
                tmp0.x = _A * tmp0.w + tmp0.x;
                tmp0.y = inp.color.w * tmp0.x + -0.5;
                tmp0.x = tmp0.x * inp.color.w;
                o.sv_target.w = tmp0.x * _Opacity;
                tmp0.x = tmp0.y < 0.0;
                if (tmp0.x) {
                    discard;
                }
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                fout o;
                float4 tmp0;
                tmp0.xy = inp.texcoord1.xy * _ParticleTextureA_ST.xy + _ParticleTextureA_ST.zw;
                tmp0 = tex2D(_ParticleTextureA, tmp0.xy);
                tmp0.x = dot(float4(_R.x, _G.x, _B.x, _A.x), tmp0);
                tmp0.x = inp.color.w * tmp0.x + -0.5;
                tmp0.x = tmp0.x < 0.0;
                if (tmp0.x) {
                    discard;
                }
