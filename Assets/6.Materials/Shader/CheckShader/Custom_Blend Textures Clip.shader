	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp3.x = tmp2.w ? 1.0 : 0.0;
                tmp3.y = _EdgeThreshold + _Threshold;
                tmp1.w = tmp1.w < tmp3.y;
                tmp1.w = tmp2.w ? tmp1.w : 0.0;
                tmp3.yzw = _Color.xyz * _EdgeColor.xyz;
                tmp3.yzw = tmp1.www ? tmp3.yzw : _Color.xyz;
                tmp2.xyz = tmp3.xxx * -tmp2.xyz + tmp2.xyz;
                tmp2.yzw = tmp3.yzw * tmp2.xyz;
                tmp1.w = tmp2.x * tmp3.y + -0.1;
                tmp1.w = tmp1.w < 0.0;
                if (tmp1.w) {
                    discard;
                }
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp3.xyz = tmp3.www * -tmp3.xyz + tmp3.xyz;
                tmp3.yzw = tmp4.xyz * tmp3.xyz;
                tmp1.w = tmp3.x * tmp4.x + -0.1;
                tmp1.w = tmp1.w < 0.0;
                if (tmp1.w) {
                    discard;
                }
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.xyz = tmp0.xxx * -tmp1.xyz + tmp1.xyz;
                tmp0.x = tmp1.x * tmp0.y + -0.1;
