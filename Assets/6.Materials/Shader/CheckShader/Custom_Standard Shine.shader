	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                o.texcoord.zw = v.texcoord.xy * _Shine_ST.xy + _Shine_ST.zw;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp4.xy = inp.texcoord.zw + _ShineOffset.xx;
                tmp4 = tex2D(_Shine, tmp4.xy);
                tmp4.xyz = tmp4.xyz * _ShineColor.xyz;
                tmp4.xyz = tmp4.xyz * _ShineGlow.xxx;
                tmp3.xyz = tmp3.xyz * _BaseGlow.xxx + tmp4.xyz;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp0;
                float4 tmp1;
                tmp0.xy = inp.texcoord.zw + _ShineOffset.xx;
                tmp0 = tex2D(_Shine, tmp0.xy);
                tmp0.xyz = tmp0.xyz * _ShineColor.xyz;
                tmp0.xyz = tmp0.xyz * _ShineGlow.xxx;
                tmp1 = tex2D(_MainTex, inp.texcoord.xy);
                tmp1.xyz = tmp1.xyz * _Color.xyz;
                tmp0.xyz = tmp1.xyz * _BaseGlow.xxx + tmp0.xyz;
