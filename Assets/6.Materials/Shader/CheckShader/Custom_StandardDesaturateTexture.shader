	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.w = dot(tmp2.xyz, float3(0.0396819, 0.4580218, 0.0060965));
                tmp2.w = 1.0 - _Saturation;
                tmp3.xyz = tmp1.www - tmp2.xyz;
                tmp2.xyz = tmp2.www * tmp3.xyz + tmp2.xyz;
                tmp2.xyz = saturate(tmp2.xyz * _Brightness.xxx);
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.w = dot(tmp3.xyz, float3(0.0396819, 0.4580218, 0.0060965));
                tmp2.w = 1.0 - _Saturation;
                tmp4.xyz = tmp1.www - tmp3.xyz;
                tmp3.xyz = tmp2.www * tmp4.xyz + tmp3.xyz;
                tmp3.xyz = saturate(tmp3.xyz * _Brightness.xxx);
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp0;
                float4 tmp1;
                tmp0.x = 1.0 - _Saturation;
                tmp1 = tex2D(_MainTex, inp.texcoord.xy);
                tmp0.y = dot(tmp1.xyz, float3(0.0396819, 0.4580218, 0.0060965));
