	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1 = tex2D(_NoiseTexture, tmp1.xy);
                tmp1.x = tmp1.x * _NoiseIntensity;
                tmp2 = tmp0 * tmp1.xxxx;
                tmp0.x = -tmp0.w * tmp1.x + 1.2;
                tmp0.x = tmp0.x > 0.99;
                if (tmp0.x) {
                    discard;
                }
                tmp0 = tmp2 * float4(1.0, 1.0, 1.0, -1.0) + float4(0.0, 0.0, 0.0, 1.2);
                o.sv_target = _Color * tmp2.wwww + tmp0;
                return o;
			}
			ENDCG
		}
