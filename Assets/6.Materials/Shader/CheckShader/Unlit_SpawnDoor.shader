	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.x = tmp0.x * _Flipper;
                tmp0.x = _Cutoff * _Flipper + -tmp0.x;
                tmp0.x = tmp0.x < 0.0;
                if (tmp0.x) {
                    discard;
                }
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                o.texcoord1.xy = v.texcoord.xy * _NoiseTex_ST.xy + _NoiseTex_ST.zw;
			...
			fout frag(v2f inp)
			{
                fout o;
                float4 tmp0;
                tmp0 = tex2D(_NoiseTex, inp.texcoord1.xy);
                tmp0.x = tmp0.x * _Flipper;
                tmp0.x = _Cutoff * _Flipper + -tmp0.x;
                tmp0.x = tmp0.x < 0.0;
                if (tmp0.x) {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1 = _Glow.xxxx * _Color;
                tmp0 = tmp0.yyyy * tmp1;
                tmp1.z = inp.texcoord.x * 0.5;
                tmp2.xy = inp.texcoord.yy * float2(0.5, 0.25);
                tmp1.xy = -_Time.xx * float2(4.0, 8.0) + tmp2.xy;
                tmp2 = tex2D(_NoiseTex, tmp1.zx);
                tmp1.w = inp.texcoord.x * 0.25 + 0.5;
                tmp1 = tex2D(_NoiseTex, tmp1.wy);
                tmp1.x = tmp1.z + tmp2.z;
                tmp1.x = tmp1.x * 1.2;
                tmp1.x = tmp1.x * tmp1.x + 0.4;
