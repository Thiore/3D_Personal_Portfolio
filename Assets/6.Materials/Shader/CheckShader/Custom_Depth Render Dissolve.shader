	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp4.y = _Time.x * 1.5 + inp.texcoord.y;
                tmp4 = tex2D(_MainTex, tmp4.xy);
                tmp5.x = inp.texcoord.x - _Time.x;
                tmp5.y = _Time.x * 2.0 + inp.texcoord.y;
                tmp5 = tex2D(_NoiseTex, tmp5.xy);
                tmp4.yzw = inp.texcoord2.xyz - _SourcePoint;
                tmp1.w = dot(tmp4.xyz, tmp4.xyz);
                tmp1.w = sqrt(tmp1.w);
                tmp2.w = tmp4.x * tmp5.x;
                tmp1.w = tmp2.w * _DissolveNoise + tmp1.w;
                tmp1.w = saturate(tmp1.w - _Dissolve);
                tmp1.w = 0.9 - tmp1.w;
                tmp1.w = tmp1.w < 0.0;
                if (tmp1.w) {
                    discard;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp5.x = inp.texcoord.x + _Time.x;
                tmp5.y = _Time.x * 1.5 + inp.texcoord.y;
                tmp5 = tex2D(_MainTex, tmp5.xy);
                tmp6.x = inp.texcoord.x - _Time.x;
                tmp6.y = _Time.x * 2.0 + inp.texcoord.y;
                tmp6 = tex2D(_NoiseTex, tmp6.xy);
                tmp5.yzw = inp.texcoord2.xyz - _SourcePoint;
                tmp1.w = dot(tmp5.xyz, tmp5.xyz);
                tmp1.w = sqrt(tmp1.w);
                tmp2.w = tmp5.x * tmp6.x;
                tmp1.w = tmp2.w * _DissolveNoise + tmp1.w;
                tmp1.w = saturate(tmp1.w - _Dissolve);
                tmp1.w = 0.9 - tmp1.w;
                tmp1.w = tmp1.w < 0.0;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp0;
                float4 tmp1;
                tmp0.x = inp.texcoord.x + _Time.x;
                tmp0.y = _Time.x * 1.5 + inp.texcoord.y;
                tmp0 = tex2D(_MainTex, tmp0.xy);
                tmp1.x = inp.texcoord.x - _Time.x;
                tmp1.y = _Time.x * 2.0 + inp.texcoord.y;
                tmp1 = tex2D(_NoiseTex, tmp1.xy);
                tmp0.x = tmp0.x * tmp1.x;
                tmp0.yzw = inp.texcoord2.xyz - _SourcePoint;
                tmp0.y = dot(tmp0.xyz, tmp0.xyz);
                tmp0.y = sqrt(tmp0.y);
                tmp0.x = tmp0.x * _DissolveNoise + tmp0.y;
                tmp0.x = saturate(tmp0.x - _Dissolve);
                tmp0.x = 0.9 - tmp0.x;
                tmp0.x = tmp0.x < 0.0;
                if (tmp0.x) {
                    discard;
