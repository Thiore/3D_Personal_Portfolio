	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp2.x = _ShineOffset * 0.1 + inp.texcoord.x;
                tmp2.yz = inp.texcoord.yx - _ShineOffset.xx;
                tmp3 = tex2D(_Shine, tmp2.xy);
                tmp2.xy = -_ShineOffset.xx * float2(2.0, 2.0) + inp.texcoord.xy;
                tmp4 = tex2D(_Shine, tmp2.xy);
                tmp2.w = -_ShineOffset * 6.0 + inp.texcoord.y;
                tmp2 = tex2D(_Shine2, tmp2.zw);
                tmp1.w = tmp3.x * tmp4.x + tmp2.x;
                tmp2.x = 1.0 - tmp1.w;
                tmp1.w = _MegaGlow * tmp2.x + tmp1.w;
                tmp2.xyz = tmp1.www * _ShineColor.xyz;
                tmp2.xyz = tmp2.xyz * _ShineGlow.xxx;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp3.x = _ShineOffset * 0.1 + inp.texcoord.x;
                tmp3.yz = inp.texcoord.yx - _ShineOffset.xx;
                tmp4 = tex2D(_Shine, tmp3.xy);
                tmp3.xy = -_ShineOffset.xx * float2(2.0, 2.0) + inp.texcoord.xy;
                tmp5 = tex2D(_Shine, tmp3.xy);
                tmp3.w = -_ShineOffset * 6.0 + inp.texcoord.y;
                tmp3 = tex2D(_Shine2, tmp3.zw);
                tmp1.w = tmp4.x * tmp5.x + tmp3.x;
                tmp2.w = 1.0 - tmp1.w;
                tmp1.w = _MegaGlow * tmp2.w + tmp1.w;
                tmp3.xyz = tmp1.www * _ShineColor.xyz;
                tmp3.xyz = tmp3.xyz * _ShineGlow.xxx;
