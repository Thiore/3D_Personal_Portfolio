	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.w = _Speed * _Time.x;
                tmp2.x = tmp1.w * 0.1 + inp.texcoord.x;
                tmp2.w = -_Time.x * _Speed + inp.texcoord.w;
                tmp2.yz = inp.texcoord.yz;
                tmp3 = tex2D(_MainTex, tmp2.xy);
                tmp2 = tex2D(_MainTex2, tmp2.zw);
                tmp2.yzw = _NightColor.xyz - _Color.xyz;
                tmp2.yzw = _Night.xxx * tmp2.yzw + _Color.xyz;
                tmp1.w = tmp2.x * 2.0 + tmp3.x;
                tmp1.w = tmp1.w * inp.color.x + -_Cutoff;
                tmp1.w = tmp1.w < 0.0;
                if (tmp1.w) {
                    discard;
                }
                ...
                tmp3.xyz = inp.texcoord1.xyz * -tmp2.xxx + -tmp1.xyz;
                ...
                tmp2.x = unity_SpecCube0_BoxMin.w < 0.99999;
                if (tmp2.x) {
                    tmp2.x = unity_SpecCube1_ProbePosition.w > 0.0;
                    if (tmp2.x) {
                        tmp2.x = dot(tmp3.xyz, tmp3.xyz);
                        tmp2.x = rsqrt(tmp2.x);
                        tmp7.xyz = tmp2.xxx * tmp3.xyz;
                        ...
                        tmp2.x = min(tmp8.y, tmp8.x);
                        tmp2.x = min(tmp8.z, tmp2.x);
                        tmp8.xyz = inp.texcoord2.xyz - unity_SpecCube1_ProbePosition.xyz;
                        tmp3.xyz = tmp7.xyz * tmp2.xxx + tmp8.xyz;
                    }
                    tmp3 = UNITY_SAMPLE_TEXCUBE_SAMPLER(unity_SpecCube0, unity_SpecCube0, float4(tmp3.xyz, 6.0));
                    tmp2.x = tmp3.w - 1.0;
                    tmp2.x = unity_SpecCube1_HDR.w * tmp2.x + 1.0;
                    tmp2.x = log(tmp2.x);
                    tmp2.x = tmp2.x * unity_SpecCube1_HDR.y;
                    tmp2.x = exp(tmp2.x);
                    tmp2.x = tmp2.x * unity_SpecCube1_HDR.x;
                    tmp3.xyz = tmp3.xyz * tmp2.xxx;
                ...
                tmp2.xyz = tmp2.yzw * float3(0.96, 0.96, 0.96);
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.w = _Speed * _Time.x;
                tmp3.x = tmp1.w * 0.1 + inp.texcoord.x;
                tmp3.w = -_Time.x * _Speed + inp.texcoord.w;
                tmp3.yz = inp.texcoord.yz;
                tmp4 = tex2D(_MainTex, tmp3.xy);
                tmp3 = tex2D(_MainTex2, tmp3.zw);
                tmp3.yzw = _NightColor.xyz - _Color.xyz;
                tmp3.yzw = _Night.xxx * tmp3.yzw + _Color.xyz;
                tmp1.w = tmp3.x * 2.0 + tmp4.x;
                tmp1.w = tmp1.w * inp.color.x + -_Cutoff;
                tmp1.w = tmp1.w < 0.0;
                if (tmp1.w) {
                    discard;
                ...
                tmp3.xyz = tmp3.yzw * float3(0.96, 0.96, 0.96);
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.x = tmp0.x * 0.1 + inp.texcoord.x;
                tmp0.yz = inp.texcoord.yz;
                tmp1 = tex2D(_MainTex, tmp0.xy);
                tmp0.w = -_Time.x * _Speed + inp.texcoord.w;
                tmp0 = tex2D(_MainTex2, tmp0.zw);
                tmp0.x = tmp0.x * 2.0 + tmp1.x;
                tmp0.x = tmp0.x * inp.color.x + -_Cutoff;
                tmp0.x = tmp0.x < 0.0;
                if (tmp0.x) {
                    discard;
                }
                tmp0.xyz = _NightColor.xyz - _Color.xyz;
                tmp0.xyz = _Night.xxx * tmp0.xyz + _Color.xyz;
