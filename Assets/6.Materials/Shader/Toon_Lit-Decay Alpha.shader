	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                float4 tmp0;
                float4 tmp1;
                tmp0 = tex2Dlod(_NoiseTex, float4(v.texcoord.xy, 0, v.texcoord.w));
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.w = inp.color.w * _Alpha;
                tmp1.w = tmp0.w * inp.color.w;
                ...
                tmp2.xyz = tmp0.xyz * inp.texcoord3.xyz;
                tmp3.x = dot(inp.texcoord1.xyz, _WorldSpaceLightPos0.xyz);
                tmp3.x = tmp3.x * 0.5 + 0.5;
                tmp3 = tex2D(_Ramp, tmp3.xx);
                tmp0.xyz = tmp0.xyz * _LightColor0.xyz;
                tmp0.xyz = tmp3.xyz * tmp0.xyz;
                tmp1.xyz = tmp0.www * tmp0.xyz;
                tmp2.w = 0.0;
                o.sv_target = tmp1 + tmp2;
                return o;
			}
			ENDCG
		}
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.xyz = inp.color.xyz * _Color.xyz;
