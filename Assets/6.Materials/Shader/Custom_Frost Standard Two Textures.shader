Shader "Custom/Frost Standard Two Textures" {
	...
	SubShader {
		...
		Pass {
			...
			float _FrostThreshold_Out;
			float _FrostThreshold_In;
			float _EdgeThreshold;
			float4 _EdgeColor;
			float _FrostNoiseScale;
			float _FrostNoiseScaleMaskingPower;
			...
			v2f vert(appdata_full v)
			{
                ...
                o.texcoord1.xy = v.texcoord.xy * _FrostTex_ST.xy + _FrostTex_ST.zw;
                o.texcoord1.zw = v.texcoord.xy * _Blend_ST.xy + _Blend_ST.zw;
			...
			fout frag(v2f inp)
			{
                ...
                tmp4 = tex2D(_Blend, inp.texcoord1.zw);
                tmp1.w = 1.0 - tmp4.x;
                tmp2.w = inp.color.x * _Threshold_Out;
                tmp1.w = tmp1.w * _Threshold_In + tmp2.w;
                tmp2.w = _Threshold < tmp1.w;
                tmp3.w = tmp2.w ? 1.0 : 0.0;
                tmp4.x = _Threshold + _EdgeThreshold;
                tmp1.w = tmp1.w < tmp4.x;
                tmp1.w = tmp2.w ? tmp1.w : 0.0;
                tmp4.xzw = _Color.xyz * _EdgeColor.xyz;
                tmp4.xzw = tmp1.www ? tmp4.xzw : _Color.xyz;
                tmp5 = tex2D(_FrostTex, inp.texcoord1.xy);
                tmp5.yz = inp.texcoord1.xy * _FrostNoiseScale.xx;
                tmp6 = tex2D(_FrostTex, tmp5.yz);
                tmp1.w = tmp5.x + tmp6.y;
                tmp2.w = saturate(tmp6.y * _FrostNoiseScaleMaskingPower + tmp4.y);
                tmp2.w = 1.0 - tmp2.w;
                tmp2.w = tmp1.w * 0.5 + tmp2.w;
                tmp4.y = inp.color.y * _FrostThreshold_Out;
                tmp2.w = tmp2.w * _FrostThreshold_In + tmp4.y;
                tmp2.w = saturate(tmp2.w - _FrostThreshold);
                tmp2.xyz = tmp2.xyz * _Tint1.xyz;
                tmp3.xyz = tmp3.xyz * _Tint2.xyz + -tmp2.xyz;
                tmp2.xyz = tmp3.www * tmp3.xyz + tmp2.xyz;
                tmp3.xyz = tmp1.www * float3(0.5, 0.5, 0.5) + _FrostColor.xyz;
                tmp3.xyz = tmp3.xyz * _FrostColor.xyz;
                tmp3.xyz = tmp2.www * tmp3.xyz;
                tmp2.xyz = tmp2.xyz * tmp4.xzw + tmp3.xyz;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp5 = tex2D(_Blend, inp.texcoord1.zw);
                tmp1.w = 1.0 - tmp5.x;
                tmp2.w = inp.color.x * _Threshold_Out;
                tmp1.w = tmp1.w * _Threshold_In + tmp2.w;
                tmp2.w = _Threshold < tmp1.w;
                tmp3.w = tmp2.w ? 1.0 : 0.0;
                tmp4.w = _Threshold + _EdgeThreshold;
                tmp1.w = tmp1.w < tmp4.w;
                tmp1.w = tmp2.w ? tmp1.w : 0.0;
                tmp5.xzw = _Color.xyz * _EdgeColor.xyz;
                tmp5.xzw = tmp1.www ? tmp5.xzw : _Color.xyz;
                tmp6 = tex2D(_FrostTex, inp.texcoord1.xy);
                tmp6.yz = inp.texcoord1.xy * _FrostNoiseScale.xx;
                tmp7 = tex2D(_FrostTex, tmp6.yz);
                tmp1.w = tmp6.x + tmp7.y;
                tmp2.w = saturate(tmp7.y * _FrostNoiseScaleMaskingPower + tmp5.y);
                tmp2.w = 1.0 - tmp2.w;
                tmp2.w = tmp1.w * 0.5 + tmp2.w;
                tmp4.w = inp.color.y * _FrostThreshold_Out;
                tmp2.w = tmp2.w * _FrostThreshold_In + tmp4.w;
                tmp2.w = saturate(tmp2.w - _FrostThreshold);
                tmp3.xyz = tmp3.xyz * _Tint1.xyz;
                tmp4.xyz = tmp4.xyz * _Tint2.xyz + -tmp3.xyz;
                tmp3.xyz = tmp3.www * tmp4.xyz + tmp3.xyz;
                tmp4.xyz = tmp1.www * float3(0.5, 0.5, 0.5) + _FrostColor.xyz;
                tmp4.xyz = tmp4.xyz * _FrostColor.xyz;
                tmp4.xyz = tmp2.www * tmp4.xyz;
                tmp3.xyz = tmp3.xyz * tmp5.xzw + tmp4.xyz;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp2;
                float4 tmp3;
                tmp0.xy = inp.texcoord1.xy * _FrostNoiseScale.xx;
                tmp0 = tex2D(_FrostTex, tmp0.xy);
                tmp1 = tex2D(_Blend, inp.texcoord1.zw);
                tmp0.x = saturate(tmp0.y * _FrostNoiseScaleMaskingPower + tmp1.y);
                tmp0.z = 1.0 - tmp1.x;
                tmp0.x = 1.0 - tmp0.x;
                tmp1 = tex2D(_FrostTex, inp.texcoord1.xy);
                tmp0.y = tmp0.y + tmp1.x;
                tmp0.x = tmp0.y * 0.5 + tmp0.x;
                tmp1.xyz = tmp0.yyy * float3(0.5, 0.5, 0.5) + _FrostColor.xyz;
                tmp1.xyz = tmp1.xyz * _FrostColor.xyz;
                tmp0.y = inp.color.y * _FrostThreshold_Out;
                tmp0.x = tmp0.x * _FrostThreshold_In + tmp0.y;
                tmp0.x = saturate(tmp0.x - _FrostThreshold);
                tmp0.xyw = tmp0.xxx * tmp1.xyz;
                tmp1.x = inp.color.x * _Threshold_Out;
                tmp0.z = tmp0.z * _Threshold_In + tmp1.x;
                tmp1.x = _Threshold + _EdgeThreshold;
                tmp1.x = tmp0.z < tmp1.x;
                tmp0.z = _Threshold < tmp0.z;
                tmp1.x = tmp0.z ? tmp1.x : 0.0;
                tmp0.z = tmp0.z ? 1.0 : 0.0;
                tmp1.yzw = _Color.xyz * _EdgeColor.xyz;
                tmp1.xyz = tmp1.xxx ? tmp1.yzw : _Color.xyz;
                tmp2 = tex2D(_MainTex2, inp.texcoord.zw);
                tmp3 = tex2D(_MainTex, inp.texcoord.xy);
                tmp3.xyz = tmp3.xyz * _Tint1.xyz;
                tmp2.xyz = tmp2.xyz * _Tint2.xyz + -tmp3.xyz;
