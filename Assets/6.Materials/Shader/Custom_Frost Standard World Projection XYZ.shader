	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1 = inp.texcoord2.xzyx / float4(_FrostScale.xxx, _WorldScale.x);
                tmp2 = tmp1.xyzz + _FrostMainVariables;
                tmp3 = tex2D(_MainTex2, tmp2.xy);
                tmp4 = tex2D(_MainTex2, tmp2.zy);
                tmp2 = tex2D(_MainTex2, tmp2.xw);
                tmp0.w = tmp3.x * abs(inp.texcoord1.y);
                tmp1.x = -tmp3.x * abs(inp.texcoord1.y) + tmp4.x;
                tmp0.w = abs(inp.texcoord1.x) * tmp1.x + tmp0.w;
                tmp1.x = tmp2.x - tmp0.w;
                tmp0.w = abs(inp.texcoord1.z) * tmp1.x + tmp0.w;
                tmp1.x = tmp1.w + _FrostNoiseVariables.x;
                tmp2.xyz = inp.texcoord2.zyy / _WorldScale.xxx;
                tmp1.yzw = tmp2.xyz + _FrostNoiseVariables.yxy;
                tmp2.x = _MaxY - _MinY;
                tmp2.y = inp.texcoord2.y - _MinY;
                tmp2.x = saturate(tmp2.y / tmp2.x);
                tmp3 = _FrostNoiseVariables * tmp1.xyzy + _FrostNoiseVariables;
                tmp1.xy = _FrostNoiseVariables.xy * tmp1.xw + _FrostNoiseVariables.zw;
                tmp4 = tex2D(_MainTex2, tmp3.xy);
                tmp1.z = tmp2.x * tmp4.y;
                tmp3 = tex2D(_MainTex2, tmp3.zw);
                tmp4 = tex2D(_MainTex2, tmp1.xy);
                tmp1.x = tmp1.z * abs(inp.texcoord1.y);
                tmp1.y = tmp3.y * tmp2.x + -tmp1.x;
                tmp1.x = abs(inp.texcoord1.x) * tmp1.y + tmp1.x;
                tmp1.y = tmp4.y * tmp2.x + -tmp1.x;
                tmp1.x = abs(inp.texcoord1.z) * tmp1.y + tmp1.x;
                tmp2 = tex2D(_Blend, inp.texcoord.zw);
                tmp0.w = tmp0.w + tmp1.x;
                tmp1.x = tmp0.w * 0.5 + -tmp2.x;
                tmp1.x = tmp1.x + 1.0;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp2 = inp.texcoord2.xzyx / float4(_FrostScale.xxx, _WorldScale.x);
                tmp3 = tmp2.xyzz + _FrostMainVariables;
                tmp4 = tex2D(_MainTex2, tmp3.xy);
                tmp5 = tex2D(_MainTex2, tmp3.zy);
                tmp3 = tex2D(_MainTex2, tmp3.xw);
                tmp0.w = tmp4.x * abs(inp.texcoord1.y);
                tmp1.w = -tmp4.x * abs(inp.texcoord1.y) + tmp5.x;
                tmp0.w = abs(inp.texcoord1.x) * tmp1.w + tmp0.w;
                tmp1.w = tmp3.x - tmp0.w;
                tmp0.w = abs(inp.texcoord1.z) * tmp1.w + tmp0.w;
                tmp2.x = tmp2.w + _FrostNoiseVariables.x;
                tmp3.xyz = inp.texcoord2.zyy / _WorldScale.xxx;
                tmp2.yzw = tmp3.xyz + _FrostNoiseVariables.yxy;
                tmp1.w = _MaxY - _MinY;
                tmp3.x = inp.texcoord2.y - _MinY;
                tmp1.w = saturate(tmp3.x / tmp1.w);
                tmp3 = _FrostNoiseVariables * tmp2.xyzy + _FrostNoiseVariables;
                tmp2.xy = _FrostNoiseVariables.xy * tmp2.xw + _FrostNoiseVariables.zw;
                tmp4 = tex2D(_MainTex2, tmp3.xy);
                tmp2.z = tmp1.w * tmp4.y;
                tmp3 = tex2D(_MainTex2, tmp3.zw);
                tmp4 = tex2D(_MainTex2, tmp2.xy);
                tmp2.x = tmp2.z * abs(inp.texcoord1.y);
                tmp2.y = tmp3.y * tmp1.w + -tmp2.x;
                tmp2.x = abs(inp.texcoord1.x) * tmp2.y + tmp2.x;
                tmp1.w = tmp4.y * tmp1.w + -tmp2.x;
                tmp1.w = abs(inp.texcoord1.z) * tmp1.w + tmp2.x;
                tmp2 = tex2D(_Blend, inp.texcoord.zw);
                tmp0.w = tmp0.w + tmp1.w;
                tmp1.w = tmp0.w * 0.5 + -tmp2.x;
                tmp1.w = tmp1.w + 1.0;
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                o.texcoord5.xyz = tmp0.xyz + tmp2.xyz;
                return o;
			}
			// Keywords: 
			fout frag(v2f inp)
			{
                ...
                float4 tmp2;
                float4 tmp3;
                tmp0.xyz = inp.texcoord2.zyy / _WorldScale.xxx;
                tmp0.yzw = tmp0.xyz + _FrostNoiseVariables.yxy;
                tmp1 = inp.texcoord2.xzyx / float4(_FrostScale.xxx, _WorldScale.x);
                tmp0.x = tmp1.w + _FrostNoiseVariables.x;
                tmp1 = tmp1.xyzz + _FrostMainVariables;
                tmp2 = _FrostNoiseVariables * tmp0.xyzy + _FrostNoiseVariables;
                tmp0.xy = _FrostNoiseVariables.xy * tmp0.xw + _FrostNoiseVariables.zw;
                tmp0 = tex2D(_MainTex2, tmp0.xy);
                tmp3 = tex2D(_MainTex2, tmp2.xy);
                tmp2 = tex2D(_MainTex2, tmp2.zw);
                tmp0.x = _MaxY - _MinY;
                tmp0.z = inp.texcoord2.y - _MinY;
                tmp0.x = saturate(tmp0.z / tmp0.x);
                tmp0.z = tmp0.x * tmp3.y;
                tmp0.z = tmp0.z * abs(inp.texcoord1.y);
                tmp0.w = tmp2.y * tmp0.x + -tmp0.z;
                tmp0.z = abs(inp.texcoord1.x) * tmp0.w + tmp0.z;
                tmp0.x = tmp0.y * tmp0.x + -tmp0.z;
                tmp0.x = abs(inp.texcoord1.z) * tmp0.x + tmp0.z;
                tmp2 = tex2D(_MainTex2, tmp1.zy);
                tmp3 = tex2D(_MainTex2, tmp1.xy);
                tmp1 = tex2D(_MainTex2, tmp1.xw);
                tmp0.y = -tmp3.x * abs(inp.texcoord1.y) + tmp2.x;
                tmp0.z = tmp3.x * abs(inp.texcoord1.y);
                tmp0.y = abs(inp.texcoord1.x) * tmp0.y + tmp0.z;
                tmp0.z = tmp1.x - tmp0.y;
                tmp0.y = abs(inp.texcoord1.z) * tmp0.z + tmp0.y;
