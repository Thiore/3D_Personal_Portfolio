	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                o.texcoord1.xy = v.texcoord.xy * _DetailMap_ST.xy + _DetailMap_ST.zw;
			...
			fout frag(v2f inp)
			{
                ...
                tmp4 = tex2D(_DetailMap, inp.texcoord1.xy);
                tmp5 = tex2D(_Blend, inp.texcoord1.zw);
                tmp1.w = tmp4.x * _DetailStrength + 1.0;
                tmp4.xyz = tmp1.www * tmp2.xyz;
                tmp2.w = 1.0 - tmp5.x;
                tmp3.w = inp.color.x * _Threshold_Out;
                tmp2.w = tmp2.w * _Threshold_In + tmp3.w;
                tmp3.w = _Threshold < tmp2.w;
                tmp4.w = tmp3.w ? 1.0 : 0.0;
                tmp5.x = _EdgeThreshold + _Threshold;
                tmp2.w = tmp2.w < tmp5.x;
                tmp2.w = tmp3.w ? tmp2.w : 0.0;
                tmp5.xyz = _Color.xyz * _EdgeColor.xyz;
                tmp5.xyz = tmp2.www ? tmp5.xyz : _Color.xyz;
                tmp2.xyz = -tmp2.xyz * tmp1.www + tmp3.xyz;
                tmp2.xyz = tmp4.www * tmp2.xyz + tmp4.xyz;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp5 = tex2D(_DetailMap, inp.texcoord1.xy);
                tmp6 = tex2D(_Blend, inp.texcoord1.zw);
                tmp1.w = tmp5.x * _DetailStrength + 1.0;
                tmp5.xyz = tmp1.www * tmp3.xyz;
                tmp2.w = 1.0 - tmp6.x;
                tmp3.w = inp.color.x * _Threshold_Out;
                tmp2.w = tmp2.w * _Threshold_In + tmp3.w;
                tmp3.w = _Threshold < tmp2.w;
                tmp4.w = tmp3.w ? 1.0 : 0.0;
                tmp5.w = _EdgeThreshold + _Threshold;
                tmp2.w = tmp2.w < tmp5.w;
                tmp2.w = tmp3.w ? tmp2.w : 0.0;
                tmp6.xyz = _Color.xyz * _EdgeColor.xyz;
                tmp6.xyz = tmp2.www ? tmp6.xyz : _Color.xyz;
                tmp3.xyz = -tmp3.xyz * tmp1.www + tmp4.xyz;
                tmp3.xyz = tmp4.www * tmp3.xyz + tmp5.xyz;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp1;
                float4 tmp2;
                tmp0 = tex2D(_Blend, inp.texcoord1.zw);
                tmp0.x = 1.0 - tmp0.x;
                ...
                tmp2 = tex2D(_DetailMap, inp.texcoord1.xy);
                tmp1.w = tmp2.x * _DetailStrength + 1.0;
                tmp2 = tex2D(_MainTex, inp.texcoord.xy);
