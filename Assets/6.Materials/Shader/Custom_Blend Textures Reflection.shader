Shader "Custom/Blend Textures Reflection" {
	...
	SubShader {
		...
		Pass {
			...
			float4 _EdgeColor;
			float4 _AltReflectionColor;
			float4 _AltReflectionColorNight;
			float _ReflectTween;
			float _Night;
			...
			fout frag(v2f inp)
			{
                ...
                tmp3 = tex2D(_MainTex2, inp.texcoord.xy);
                tmp4 = tex2D(_Blend, inp.texcoord.zw);
                tmp1.w = 1.0 - tmp4.x;
                tmp3.w = inp.color.x * _Threshold_Out;
                tmp1.w = tmp1.w * _Threshold_In + tmp3.w;
                tmp3.w = _Threshold < tmp1.w;
                tmp4.x = tmp3.w ? 1.0 : 0.0;
                tmp4.y = _EdgeThreshold + _Threshold;
                tmp1.w = tmp1.w < tmp4.y;
                tmp1.w = tmp3.w ? tmp1.w : 0.0;
                tmp4.yzw = _Color.xyz * _EdgeColor.xyz;
                tmp4.yzw = tmp1.www ? tmp4.yzw : _Color.xyz;
                tmp2.xyz = tmp2.xyz * _Tint1.xyz;
                tmp3.xyz = tmp3.xyz * _Tint2.xyz + -tmp2.xyz;
                tmp2.xyz = tmp4.xxx * tmp3.xyz + tmp2.xyz;
                tmp2.xyz = tmp4.yzw * tmp2.xyz;
                tmp3.xy = inp.texcoord3.xy / inp.texcoord3.ww;
                tmp3 = tex2D(_ReflectionTex, tmp3.xy);
                tmp3.xyz = tmp2.www * tmp3.xyz;
                tmp4.yzw = _AltReflectionColorNight.xyz - _AltReflectionColor.xyz;
                tmp4.yzw = _Night.xxx * tmp4.yzw + _AltReflectionColor.xyz;
                tmp3.xyz = tmp3.xyz * _ReflectionAlpha.xxx + -tmp4.yzw;
                tmp3.xyz = _ReflectTween.xxx * tmp3.xyz + tmp4.yzw;
                tmp1.w = unity_ProbeVolumeParams.x == 1.0;
                if (tmp1.w) {
                    tmp1.w = unity_ProbeVolumeParams.y == 1.0;
                    tmp4.yzw = inp.texcoord2.yyy * unity_ProbeVolumeWorldToObject._m01_m11_m21;
                    tmp4.yzw = unity_ProbeVolumeWorldToObject._m00_m10_m20 * inp.texcoord2.xxx + tmp4.yzw;
                    tmp4.yzw = unity_ProbeVolumeWorldToObject._m02_m12_m22 * inp.texcoord2.zzz + tmp4.yzw;
                    tmp4.yzw = tmp4.yzw + unity_ProbeVolumeWorldToObject._m03_m13_m23;
                    tmp4.yzw = tmp1.www ? tmp4.yzw : inp.texcoord2.xyz;
                ...
                if (tmp2.w) {
                    ...
                    if (tmp2.w) {
                        ...
                        tmp4.yzw = tmp8.xyz * tmp2.www + tmp9.xyz;
                    }
                    tmp8 = UNITY_SAMPLE_TEXCUBE_SAMPLER(unity_SpecCube0, unity_SpecCube0, float4(tmp4.yzw, 6.0));
                    tmp2.w = tmp8.w - 1.0;
                    ...
                    tmp4.yzw = tmp8.xyz * tmp2.www;
                    tmp6.xyz = tmp1.www * tmp6.xyz + -tmp4.yzw;
                    tmp7.xyz = unity_SpecCube0_BoxMin.www * tmp6.xyz + tmp4.yzw;
                }
                tmp1.w = dot(inp.texcoord1.xyz, inp.texcoord1.xyz);
                tmp1.w = rsqrt(tmp1.w);
                tmp4.yzw = tmp1.www * inp.texcoord1.xyz;
                ...
                tmp1.xzw = tmp7.xyz * float3(0.5, 0.5, 0.5);
                tmp0.w = tmp1.y * 0.0 + 0.04;
                tmp0.xyz = tmp1.xzw * tmp0.www + tmp0.xyz;
                o.sv_target.xyz = tmp4.xxx * tmp3.xyz + tmp0.xyz;
                o.sv_target.w = 1.0;
                return o;
			}
			ENDCG
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp4 = tex2D(_MainTex2, inp.texcoord.xy);
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                float4 tmp1;
                float4 tmp2;
                tmp0 = tex2D(_Blend, inp.texcoord.zw);
                tmp0.x = 1.0 - tmp0.x;
                tmp0.y = inp.color.x * _Threshold_Out;
                tmp0.x = tmp0.x * _Threshold_In + tmp0.y;
                ...
                tmp1 = tex2D(_MainTex2, inp.texcoord.xy);
                tmp2 = tex2D(_MainTex, inp.texcoord.xy);
                tmp2.xyz = tmp2.xyz * _Tint1.xyz;
                tmp1.xyz = tmp1.xyz * _Tint2.xyz + -tmp2.xyz;
                tmp1.xyz = tmp0.xxx * tmp1.xyz + tmp2.xyz;
                tmp0.yzw = tmp0.yzw * tmp1.xyz;
                o.sv_target.xyz = tmp0.yzw * float3(0.96, 0.96, 0.96);
                o.sv_target.w = 1.0;
                ...
                tmp1.xyz = _AltReflectionColorNight.xyz - _AltReflectionColor.xyz;
                tmp1.xyz = _Night.xxx * tmp1.xyz + _AltReflectionColor.xyz;
                tmp0.yzw = tmp0.yzw * _ReflectionAlpha.xxx + -tmp1.xyz;
                tmp0.yzw = _ReflectTween.xxx * tmp0.yzw + tmp1.xyz;
