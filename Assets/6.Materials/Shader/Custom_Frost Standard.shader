	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                o.texcoord1.xy = v.texcoord.xy * _MainTex3_ST.xy + _MainTex3_ST.zw;
			...
			fout frag(v2f inp)
			{
                ...
                tmp4 = tex2D(_MainTex3, inp.texcoord1.xy);
                tmp5 = tex2D(_Blend, inp.texcoord1.zw);
                tmp1.w = tmp3.x + tmp4.x;
                tmp2.w = tmp1.w * 0.5 + -tmp5.x;
                tmp2.w = tmp2.w + 1.0;
                tmp3.x = inp.color.x * _Threshold_Out;
                tmp2.w = tmp2.w * _Threshold_In + tmp3.x;
                tmp2.w = saturate(tmp2.w - _Threshold);
                tmp3.xyz = tmp1.www * float3(0.5, 0.5, 0.5) + _Tint2.xyz;
                tmp3.xyz = tmp3.xyz * _Tint2.xyz;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp5 = tex2D(_MainTex3, inp.texcoord1.xy);
                tmp6 = tex2D(_Blend, inp.texcoord1.zw);
                tmp1.w = tmp4.x + tmp5.x;
                tmp2.w = tmp1.w * 0.5 + -tmp6.x;
                tmp2.w = tmp2.w + 1.0;
                tmp3.w = inp.color.x * _Threshold_Out;
                tmp2.w = tmp2.w * _Threshold_In + tmp3.w;
                tmp2.w = saturate(tmp2.w - _Threshold);
                tmp4.xyz = tmp1.www * float3(0.5, 0.5, 0.5) + _Tint2.xyz;
                tmp4.xyz = tmp4.xyz * _Tint2.xyz;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp2 = tex2D(_MainTex3, inp.texcoord1.xy);
                tmp0.y = tmp1.x + tmp2.x;
                tmp0.x = tmp0.y * 0.5 + -tmp0.x;
                tmp0.yzw = tmp0.yyy * float3(0.5, 0.5, 0.5) + _Tint2.xyz;
                tmp0.yzw = tmp0.yzw * _Tint2.xyz;
                tmp0.x = tmp0.x + 1.0;
                tmp1.x = inp.color.x * _Threshold_Out;
                tmp0.x = tmp0.x * _Threshold_In + tmp1.x;
                tmp0.x = saturate(tmp0.x - _Threshold);
                tmp0.xyz = tmp0.xxx * tmp0.yzw;
