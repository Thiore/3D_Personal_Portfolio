	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                while (true) {
                    ...
                    tmp0.z = floor(i);
                    tmp1.x = tmp0.z * _Scale + v.texcoord.x;
                    tmp0.z = tmp0.x;
                    tmp0.w = NaN;
                    for (int j = tmp0.w; j <= 1; j += 1) {
                        tmp1.z = floor(j);
                        tmp1.y = tmp1.z * _Scale + v.texcoord.y;
                        tmp2 = tex2Dlod(_ImprintMap, float4(tmp1.xy, 0, v.texcoord.w));
                        tmp0.z = tmp0.z + tmp2.x;
                    }
                    tmp0.x = tmp0.z;
                    i = i + 1;
                }
                tmp0.x = tmp0.x * 0.1111111;
                ...
                i = dot(v.normal.xyz, unity_WorldToObject._m01_m11_m21);
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                tmp2.x = rsqrt(tmp2.x);
                o.texcoord1.xyz = tmp0.xyz * tmp2.xxx;
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                tmp1 = unity_ObjectToWorld._m02_m12_m22_m32 * tmp0.zzzz + tmp1;
                tmp0 = unity_ObjectToWorld._m03_m13_m23_m33 * tmp0.wwww + tmp1;
                tmp1.x = unity_LightShadowBias.z != 0.0;
                ...
                tmp1.yzw = -tmp1.yzw * tmp2.xxx + tmp0.xyz;
