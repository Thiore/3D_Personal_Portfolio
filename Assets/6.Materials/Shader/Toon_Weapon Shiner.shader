	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0 = tmp0 * _Color;
                tmp1 = tex2D(_Noise, inp.texcoord.zw);
                tmp1.yzw = inp.texcoord2.xyz - _SourcePoint;
                tmp1.y = dot(tmp1.xyz, tmp1.xyz);
                tmp1.y = sqrt(tmp1.y);
                tmp1.x = tmp1.x + tmp1.y;
                tmp1.x = tmp1.x - _ShineOffset;
                tmp1.y = _MinDist < tmp1.x;
                tmp1.x = tmp1.x < _MaxDist;
                tmp1.x = tmp1.x ? tmp1.y : 0.0;
                tmp1.yzw = _ShineColor.xyz * _ShineGlow.xxx;
                tmp1.xyz = tmp1.xxx ? tmp1.yzw : 0.0;
                tmp0.xyz = tmp0.xyz * _Color.xyz;
                tmp2.xyz = _GlowColor.xyz * _Glow.xxx + -tmp0.xyz;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1 = tmp1 * _Color;
                tmp2 = tex2D(_Noise, inp.texcoord.zw);
                tmp2.yzw = inp.texcoord2.xyz - _SourcePoint;
                tmp0.w = dot(tmp2.xyz, tmp2.xyz);
                tmp0.w = sqrt(tmp0.w);
                tmp0.w = tmp2.x + tmp0.w;
                tmp0.w = tmp0.w - _ShineOffset;
                tmp2.x = _MinDist < tmp0.w;
                tmp0.w = tmp0.w < _MaxDist;
                tmp0.w = tmp0.w ? tmp2.x : 0.0;
                tmp2.xyz = _ShineColor.xyz * _ShineGlow.xxx;
                tmp2.xyz = tmp0.www ? tmp2.xyz : 0.0;
                tmp1.xyz = tmp1.xyz * _Color.xyz;
                tmp3.xyz = _GlowColor.xyz * _Glow.xxx + -tmp1.xyz;
