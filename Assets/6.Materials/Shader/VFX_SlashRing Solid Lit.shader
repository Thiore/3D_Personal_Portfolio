	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata_full v)
			{
                ...
                o.texcoord.zw = v.texcoord.xy * _GlowTex_ST.xy + _GlowTex_ST.zw;
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.w = tmp2.x * tmp2.x + -_Cutoff;
                tmp1.w = tmp1.w < 0.0;
                if (tmp1.w) {
                    discard;
                }
                tmp1.w = tmp2.y - _SolidCutoff;
                tmp1.w = tmp1.w > 0.0;
                tmp1.w = tmp1.w ? 1.0 : 0.0;
                tmp2.xyz = tmp1.www * _SolidColor.xyz;
                tmp3 = tex2D(_GlowTex, inp.texcoord.zw);
                tmp4.xyz = _Color.xyz * _Glow.xxx;
                tmp4.xyz = tmp1.www * -tmp4.xyz + tmp4.xyz;
                tmp5.xyz = _Glow.xxx - tmp4.xyz;
                tmp3.xyz = tmp3.xyz * tmp5.xyz + tmp4.xyz;
                ...
                if (tmp2.w) {
                    ...
                    if (tmp2.w) {
                        ...
                        tmp4.xyz = tmp8.xyz * tmp2.www + tmp9.xyz;
                    }
                    ...
                    tmp6.xyz = tmp1.www * tmp6.xyz + -tmp4.xyz;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp1.w = tmp3.x * tmp3.x + -_Cutoff;
                tmp1.w = tmp1.w < 0.0;
                if (tmp1.w) {
                    discard;
                }
                tmp1.w = tmp3.y - _SolidCutoff;
                tmp1.w = tmp1.w > 0.0;
                tmp1.w = tmp1.w ? 1.0 : 0.0;
                tmp3.xyz = tmp1.www * _SolidColor.xyz;
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                ...
                tmp0.xzw = tmp0.yyy * _SolidColor.xyz;
                o.sv_target.xyz = tmp0.xzw * float3(0.96, 0.96, 0.96);
                o.sv_target.w = 1.0;
                o.sv_target1 = float4(0.04, 0.04, 0.04, 0.0);
                o.sv_target2.xyz = inp.texcoord1.xyz * float3(0.5, 0.5, 0.5) + float3(0.5, 0.5, 0.5);
                o.sv_target2.w = 1.0;
                tmp0.xzw = _Color.xyz * _Glow.xxx;
                tmp0.xyz = tmp0.yyy * -tmp0.xzw + tmp0.xzw;
                tmp1.xyz = _Glow.xxx - tmp0.xyz;
                tmp2 = tex2D(_GlowTex, inp.texcoord.zw);
